...
type ActMarkupRepurchaseFoodListResponseData struct {
	ActId           int64                                                 `json:"act_id"`            //描述：活动id
	ActMame         string                                                `json:"act_name"`          //描述：活动名称，不超过12个字符。
	...
	AppFoods        []ActMarkupRepurchaseFoodListResponseDataAppFoodsItem `json:"app_foods"`         //描述：参加换购活动商品数据，json格式数组。 注意：app_food_code即为药品的app_medicine_code。
}
