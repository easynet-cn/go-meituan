...
type ActGoodsCouponGoodslistRequestDataItem struct {
	AppPoiCodes         string   `json:"app_poi_codes"`          //描述：可使用券的门店id列表，多个用英文逗号分隔。传APP方门店id，即商家中台系统里门店的编码。如商家在操作绑定门店至开放平台应用中时，未绑定三方门店id信息，则默认APP方门店id与美团门店id相同。
	...
	AppFoodCodes        string   `json:"app_food_codes"`         //描述：参加活动的商品，传APP方商品id，即商家中台系统里商品的编码（spu_code值）。如不同门店内同一商品的app_food_code不同，则视为不同商品，多个商品id用英文逗号分隔。 注意：app_food_code即为药品的app_medicine_code。
