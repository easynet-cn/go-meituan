...
type ActGoodsCouponSaveRequestGoodsCouponData struct {
	...
	Type                int      `json:"id"`                     //是否必须：是；描述：商品券类型，取值范围：1按活动价格；2按折扣。
	ActPriceCouponInfo  []string `json:"act_price_coupon_info"`  //是否必须：是；描述：按活动价格的商品券信息，json格式数组。按活动价格的商品券活动最多支持设置三层优惠。
	DiscountCouponInfo  []string `json:"discount_coupon_info"`   //是否必须：是；描述：按折扣的商品券信息，json格式数组。(1)当discount大于0时，折扣商品券活动的优惠信息不能超过三层。(2)当discount等于0时，为商品兑换券活动，仅支持设置一层。
}
