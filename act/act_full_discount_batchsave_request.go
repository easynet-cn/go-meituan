...
type ActFullDiscountBatchsaveRequest struct {
	...
	ActDetails string `url:"act_details"`  //是否必须：是；描述：活动详情信息，json格式数组。活动满减不能超过5层，参数包括：(1)origin_price(必填项)，表示满的金额 ；(2)act_price(必填项)，表示减的金额。满、减金额都必须是大于0的整数，单位是元。举例：origin_price=20，act_price=2，表示满20元减2元。
