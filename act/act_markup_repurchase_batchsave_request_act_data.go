...
type ActMarkupRrepurchaseBatchsaveRequestActData struct {
	...
	OrderLimitTotal *int                                                      `json:"order_limit_total"` //是否必须：否；描述：每单可换购的商品总数，须为大于0的整数或-1，传-1则表示不限制。更新活动时，不支持修改此信息。
	AppFoods        []ActMarkupRrepurchaseBatchsaveRequestActDataAppFoodsItem `json:"app_foods"`         //是否必须：否，创建时必填；描述：参加换购的活动商品数据，json格式；同一次调用中，可上传的活动商品数据不能超过200组。更新活动时，支持添加活动商品；支持修改已有活动商品的相关信息。 注意：app_food_code即为药品的app_medicine_code。
}
