...
type ActGoodsCouponSaveRequestGoodsCouponDataDiscountCouponInfo struct {
	FullPrice      *int    `json:"full_price"`       //是否必须：否；描述：券使用门槛金额，范围是1~999的正整数，单位元。(1)当discount大于0时，此字段必传，折扣商品券活动的优惠信息不能超过三层。(2)当discount等于0时，为商品兑换券活动，不支持设置门槛金额，full_price默认为0。
