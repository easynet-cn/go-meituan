...
type MedicineCategory struct {
	...
	Sequence           string `json:"sequence"`             //描述：一级分类在当前门店内的排序序号，数字越小，前端排位越靠前。
	...
	SecondSequence     string `json:"second_sequence"`      //描述：二级分类在所属一级分类下的排序序号，数字越小，前端排位越靠前。
	CTime              string `json:"ctime"`                //描述：创建一级分类的时间，为10位秒级的时间戳。
	UTime              string `json:"utime"`                //描述：最近一次修改一级分类的时间，为10位秒级的时间戳。
}
